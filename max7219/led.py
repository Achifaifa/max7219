...
class device(object):
  ...
  def __init__(self, cascaded=1, spi_bus=0, spi_device=0):
    ...
    self.command(constants.scanlimit, 7)  # show all 8 digits
    self.command(constants.decodemode, 0)   # use matrix (not digits)
    self.command(constants.displaytest, 0)  # no display test
    self.command(constants.shutdown, 1)   # not shutdown mode
  ...
  def command(self, register, data):
    ...
    assert constants.decodemode<=register<=constants.displaytest
  ...
  def brightness(self, intensity):
    ...
    self.command(constants.intensity, intensity)
...
class matrix(device):
  ...
  def pixel(self, x, y, value, redraw=True):
    ...
    if value: self._buffer[x] |= (1 << y)
    else: self._buffer[x] &= ~(1 << y)
    if redraw: self.flush()
  ...
  def _preprocess_buffer(self, buf):
    ...
    if self._invert: buf=[~x & 0xff for x in buf]
    if self._orientation: buf=self._rotate(buf)
